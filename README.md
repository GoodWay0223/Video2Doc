...
加载 Skill 后，Agent 会自动展示引导菜单，包括：

### ① 选择输出模式
> 📝 纯文稿（默认）vs 🎨 深度分析

### ② 配置转录引擎（首次必走）
> ☁️ 硅基流动 API（推荐，免费 10h/月）vs 💻 本地 Whisper

如果还没有硅基流动账号，Agent 会引导注册：
> **注册链接**：https://cloud.siliconflow.cn/i/pWcvZzOr

注册后在"API 密钥"页面创建 Key，告诉 Agent 即可自动保存到
`~/.workbuddy/MEMORY.md`，以后就不用再配置了。
